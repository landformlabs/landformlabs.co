...
export default function DesignConfigurator({
  ...
  const colorOptions = [
    ...
    { name: "Sky Blue", value: "#3b82f6", bg: "bg-blue-500" },
  ...
  useEffect(() => {
    const loadTopographyData = async () => {
      ...
      try {
        ...
        setTopoError('Unable to load topography data. Preview will show route only.');
  ...
  useEffect(() => {
    ...
    const filteredPoints = gpxData.points.filter((point: any) =>
              ...
              {isLoadingTopo && (
                <div className="absolute inset-0 bg-white/80 flex items-center justify-center rounded-lg">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-slate-storm">Loading topography...</p>
                  </div>
                </div>
              )}
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 sticky top-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {colorOptions.map((color) => (
                <button
                  key={color.value}
                  onClick={() => handleConfigChange({ routeColor: color.value })}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="font-semibold text-basalt text-sm mb-1">Route Tile</div>
                <div className="text-xs text-slate-storm">Square display piece</div>
              </button>
              <button
              ...
              >
                <div className="font-semibold text-basalt text-sm mb-1">Ornament</div>
                <div className="text-xs text-slate-storm">Circular hanging piece</div>
              </button>
            </div>
          </div>
              ...
              {designConfig.labels.map((label, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-basalt">{label.text}</div>
                    <div className="text-xs text-slate-storm">Size: {label.size}px</div>
                  </div>
                  <button
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    max="32"
                    value={newLabel.size}
                    onChange={(e) => setNewLabel({ ...newLabel, size: parseInt(e.target.value) })}
                    className="flex-1"
                  />
                  <span className="text-xs text-slate-storm whitespace-nowrap">{newLabel.size}px</span>
                </div>
                <button
                  onClick={addLabel}
